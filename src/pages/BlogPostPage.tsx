...
import { resolveActiveSectionId, scrollToArticleSection } from '../lib/articleScrollOffset';
...
export const BlogPostPage: React.FC = () => {
  ...
  const tocItems = useMemo(() => (article ? extractArticleToc(article.content) : []), [article]);

  const [copied, setCopied] = useState(false);
  const [isBookmarked, setIsBookmarked] = useState(false);
  const [scrollTocId, setScrollTocId] = useState<string | undefined>();
  const activeTocId =
    scrollTocId && tocItems.some((item) => item.id === scrollTocId) ? scrollTocId : tocItems[0]?.id;
  ...
  useEffect(() => {
    ...
    const link = nav.querySelector<HTMLAnchorElement>(`a[href="#${CSS.escape(activeTocId)}"]`);
    link?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  }, [activeTocId]);

  const handleTocNavigate = (event: React.MouseEvent<HTMLAnchorElement>, sectionId: string) => {
                        ...
                        {tocItems.map((item) => (
                          ...
                          >
                            <a
                              ...
                              className={
                                activeTocId === item.id
                                  ? 'blog-post-page__toc-link--active'
                                  : undefined
                              }
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        >
                          <p className="blog-post-page__authorline">
                            ...
                            <span className="blog-post-page__author-name">
                              {article.authorName}
                            </span>
                            {article.authorRole ? (
                              <span className="blog-post-page__author-role">
                                , {article.authorRole}
                              </span>
                            ) : null}
                          </p>
                          ...
                          <AnimatePresence>
                            {copied && (
                              <motion.div
                                ...
                                className="absolute -top-11 left-1/2 z-10 -translate-x-1/2 whitespace-nowrap rounded-lg bg-text px-3 py-1.5 text-xs font-bold uppercase tracking-widest text-white"
                              >
                                Copied
                              </motion.div>
                            )}
                ...
                {data.articles
                  ...
                  .map((rel) => (
                    <Link
                      key={rel.id}
                      to={`/blog/${rel.slug}`}
                      className="blog-post-related__card group"
                    >
                      <div className="blog-post-related__card-inner">
                        <div className="blog-post-related__media">
                          ...
                          <div className="blog-post-related__badge-wrap">
                            <span className="blog-post-related__badge">{rel.category}</span>
                          </div>
                        </div>
                        <div className="blog-post-related__body">
                          <h3 className="blog-post-related__card-title">{rel.title}</h3>
