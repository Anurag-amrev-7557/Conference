...
/** Cloudinary-hosted summit highlight (replaces local / API hero mp4 in production). */
export const DEFAULT_CONFERENCE_CLOUDINARY_VIDEO =
  'https://res.cloudinary.com/dtvehewoe/video/upload/v1781188866/Conference_Video_1920x1080_izrbvx.mp4';
...
/** Static fallback when CMS/env video URL is empty (local mp4 stripped from production builds). */
export const CONFERENCE_HERO_VIDEO_PUBLIC = DEFAULT_CONFERENCE_CLOUDINARY_VIDEO;
...
export const DEFAULT_CONFERENCE_VIDEO = DEFAULT_CONFERENCE_CLOUDINARY_VIDEO;
...
export const defaultConferenceContent: ConferenceContent = {
  ...
  venue: {
    ...
    title: 'Where we',
    titleAccent: 'gather',
    lede: 'Marymoor Park, Redmond, Washington — an open-air setting built for keynotes, workshops, and high-signal networking.',
    address: '6046 West Lake Sammamish Pkwy NE, Redmond, WA 98052',
    mapEmbedUrl:
      'https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2689.0!2d-122.116!3d47.663!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x54906c8c8c8c8c8d%3A0x0!2sMarymoor%20Park!5e0!3m2!1sen!2sus!4v1',
  },
  hero: {
  ...
  video: {
    ...
    lede: 'Step inside the energy of Superhumanly Summit — keynotes, workshops, and the conversations that shape what ships next.',
    videoUrl: DEFAULT_CONFERENCE_VIDEO,
    posterUrl: DEFAULT_CONFERENCE_POSTER,
  },
  sections: {
    ...
    faq: {
      ...
      lede: 'Everything you need to know before you register.',
    },
    tickets: {
      eyebrow: 'Passes',
      title: 'Secure Your',
    ...
    },
    {
      ...
      sessions: [
        ...
        },
        {
          ...
          room: 'Main Hall',
          description:
            'Celebrating standout summit projects and closing notes from the NeuralNet team.',
        },
      ],
    },
  ],
...
function mergeSection(
  ...
  const merged: ConferenceSectionCopy = {
    ...base,
    ...patch,
    metrics: patch?.metrics?.length ? patch.metrics : base?.metrics,
  };

  // CMS often stores only the accent word as title (e.g. "Agenda") while defaults
  // still carry titleAccent — restore the intended split instead of "Agenda Agenda".
  if (patch?.title?.trim() && !('titleAccent' in patch)) {
    const patchTitle = patch.title.trim();
    const baseTitle = base?.title?.trim();
    const baseAccent = base?.titleAccent?.trim();
    if (
      baseAccent &&
      patchTitle.toLowerCase() === baseAccent.toLowerCase() &&
      baseTitle &&
      baseTitle.toLowerCase() !== patchTitle.toLowerCase()
    ) {
      merged.title = baseTitle;
      merged.titleAccent = baseAccent;
    }
  }
...
function mergeHero(
  base: ConferenceHeroContent,
  patch?: Partial<ConferenceHeroContent>,
): ConferenceHeroContent {
  const merged = !patch
    ? base
    : {
