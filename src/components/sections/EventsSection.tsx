...
function EventPreviewCard({ event, index, cardCtaUpcoming, cardCtaPast }: EventPreviewCardProps) {
  ...
  const {
    ...
    initials,
  } = formatEventHost(event.host, event.location);
  ...
  return (
    <div className="events-carousel-card" style={{ '--event-i': index } as CSSProperties}>
      <Link to={eventHref} className="events-carousel-card__link group" aria-label={linkLabel}>
          ...
          <div className="events-carousel-card__body">
            <div className="events-carousel-card__meta-row">
              ...
              <span className="events-carousel-card__meta-end">
                {timeOnly ? <span className="events-carousel-card__time">{timeOnly}</span> : null}
                {event.status === 'Past' ? (
            ...
            <div className="events-carousel-card__footer">
              <div className="events-carousel-card__host">
                ...
                <span className="events-carousel-card__host-text">
                  ...
                  <span className="events-carousel-card__host-location">{hostSubtitle}</span>
                </span>
              </div>
              ...
              <div className="events-carousel-card__tags">
                ...
                {event.tags.slice(0, 2).map((tag) => (
                  <span key={tag.name} className={cn('events-carousel-card__tag', tag.color)}>
                    {tag.name}
                  </span>
                ))}
                {event.price ? (
...
export function EventsSection({ className }: { className?: string }) {
  ...
  const previewCount = preview?.previewCount && preview.previewCount > 0 ? preview.previewCount : 4;
    ...
    >
      ...
      <div className="relative z-10 w-full px-5 sm:px-8 lg:px-12 xl:px-16 2xl:px-20 max-w-[1600px] mx-auto">
        <EditorialSectionHeader
          ...
          eyebrowFallback="Founder Calendar"
          ledeFallback={DEFAULT_LEDE}
          centered
          className="events-section__header flex flex-col items-center text-center max-w-4xl mx-auto mb-12 sm:mb-14 lg:mb-16"
        />
        ...
        {previewEvents.length > 0 ? (
          ...
          <div className="events-section__empty">
            ...
            <Link to={preview?.ctaHref?.trim() || '/events'} className="events-section__empty-cta">
